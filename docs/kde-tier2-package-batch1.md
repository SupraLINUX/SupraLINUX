...
KAuth is therefore downstream-eligible. The original Batch 1 promotion record captured **1 PASS / 1 pending** because the then-current discovery contained only KAuth + KMime. After correcting the upstream Tier 2 inventory to 15 nodes, the current canonical package state is **1 PASS / 14 pending / 0 current FAIL / 0 BLOCKED**. The 13 newly discovered non-KMime nodes remain package-state pending, but later readiness may advance independently of this historical KAuth batch. Five are currently package-contract-ready after provider audit, while eight still await provider/profile audit. KMime remains decision-gated by ADR-0002.
