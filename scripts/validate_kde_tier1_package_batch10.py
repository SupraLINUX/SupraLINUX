...
req(c.get('state')=='remediation-pending-build','Batch 10 campaign must remain remediation-pending while QuickCharts is unresolved')
...
versions={'kirigami':'6.30.0-0supralinux2','kquickcharts':'6.30.0-0supralinux2'}
for node,(source,sha,nbin,nabi,tests,job,artifact,digest) in expected.items():
 ...
 req(n.get('downstream_eligible') is (node=='kirigami'),f'{node}: downstream-eligibility mismatch for retained campaign state')
...
req(quick.get('state')=='remediation-pending-build' and quick.get('last_result')=='FAIL' and quick.get('downstream_eligible') is False,'KQuickCharts remediation state mismatch')
...
req(qf.get('validation_cycle')==3 and qf.get('workflow_run')==35398956698 and qf.get('job_id')==105776448807 and qf.get('artifact_id')==10570203712 and qf.get('artifact_sha256')=='1f4a4a65e3e534d000b0507ff583f8ef657eb5836d0db840ef405d5ab501f12e' and qf.get('rootfs_sha256')=='7da69856573c2d320f7e8a345d53fff793c39e0e8fe1e0e144bed73fcf3765d2' and qf.get('tests')=='8/8 PASS' and qf.get('stage')=='sbuild' and qf.get('kirigami_predecessor_version')=='6.30.0-0supralinux2','KQuickCharts attempt-1 failure evidence mismatch')
...
req(len(kp)==1 and kp[0].get('node')=='kirigami' and kp[0].get('kind')=='qml-runtime-closure' and kp[0].get('not_kde_framework_build_dependency') is True and kp[0].get('state')=='retained-PASS-available','QuickCharts Kirigami package-validation dependency mismatch')
...
req(len(ka)==2 and len(qa)==1,'Batch 10 real-attempt ledger count mismatch')
if len(ka)==2:
 x=ka[0]
...
for token in ('25 PASS / 4 pending','DIAG_PASS','10464419377','10463808939','package_validation_dependency','BLOCKED','35398956698','10569258322','10570203712','44/44 PASS','8/8 PASS','optional=templinst|arch=!riscv64','KF6::KirigamiPlatform','qml6-module-org-kde-kirigami'):
...
print('Nodes: kirigami retained PASS; kquickcharts remediation-pending-build with retained Kirigami PASS available')
