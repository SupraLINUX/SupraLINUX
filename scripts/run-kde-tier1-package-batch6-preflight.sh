...
CAMPAIGN="${ROOT}/manifests/kde-tier1-package-campaign-batch6.json"
NODE="${1:-}"
...
shared=data['shared_predecessors']; signing=node['signing_key']; symbols=node['symbols']
vals={
...
WORK_DIR="${ROOT}/.work/kde-tier1-package-batch6/${NODE}"
SOURCE_WORK="${WORK_DIR}/source"
OUT_DIR="${WORK_DIR}/out"
EVIDENCE_DIR="${ROOT}/evidence/kde-tier1-package-batch6/${NODE}"
...
write_result() {
  ...
  python3 - "${RESULT_JSON}" "${NODE}" "${STATE}" "${rc}" "${STAGE}" "${STARTED_AT}" "${finished_at}" "${UPSTREAM_VERSION}" "${DEBIAN_VERSION}" <<'PY2'
import json,sys
from pathlib import Path
...
echo "=== SupraLINUX KDE Tier 1 Batch 6: ${NODE} ${UPSTREAM_VERSION} (${DEBIAN_VERSION}) ==="
...
sudo apt-get update
sudo DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends binutils ca-certificates cmake curl devscripts dpkg-dev g++ libimobiledevice-dev libmount-dev libplist-dev libudev-dev lintian mmdebstrap ninja-build pkg-config qt6-base-dev qt6-declarative-dev qt6-wayland-dev sbuild uidmap ubuntu-keyring xz-utils
...
{ cat /etc/os-release; uname -a; sbuild --version; mmdebstrap --version; lintian --version; apt-cache policy plasma-wayland-protocols wayland-protocols libmount-dev libudev-dev libimobiledevice-dev libplist-dev; } > "${EVIDENCE_DIR}/host.txt"
...
from pathlib import Path
campaign,node_id,out_dir=sys.argv[1:]; node=json.loads(Path(campaign).read_text())['nodes'][node_id]; debs={}
...
STATE="PASS"; STAGE="complete"; echo "KDE Tier 1 Batch 6 node ${NODE}: PASS"
