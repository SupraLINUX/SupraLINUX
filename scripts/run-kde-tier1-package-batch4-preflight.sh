...
WORK_DIR="${ROOT}/.work/kde-tier1-package-batch4/${NODE}"
SOURCE_WORK="${WORK_DIR}/source"
OUT_DIR="${WORK_DIR}/out"
EVIDENCE_DIR="${ROOT}/evidence/kde-tier1-package-batch4/${NODE}"
...
echo "=== SupraLINUX KDE Tier 1 Batch 4: ${NODE} ${UPSTREAM_VERSION} (${DEBIAN_VERSION}) ==="
...
sudo apt-get update
sudo DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends binutils ca-certificates cmake curl devscripts dpkg-dev g++ lintian mmdebstrap ninja-build pkg-config qt6-base-dev qt6-declarative-dev sbuild uidmap ubuntu-keyring xz-utils
...
STATE="PASS"; STAGE="complete"; echo "KDE Tier 1 Batch 4 node ${NODE}: PASS"
