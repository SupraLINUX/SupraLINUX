...
WORK="${ROOT}/.work/kde-tier3-kio-round20-diagnostic"
INPUTS="${WORK}/inputs"
APT_REPO="/tmp/supralinux-round20-repo"
EVIDENCE="${ROOT}/evidence/kde-tier3-kio-round20-diagnostic"
...
Path(p).write_text(json.dumps({
 "schema":1,"node":"kio","round":20,"diagnostic_result":result,
...
STAGE=contract
python3 scripts/validate_kde_tier3_kio_round20_diagnostic.py
...
echo "deb [trusted=yes] file:${APT_REPO} ./" | sudo tee /etc/apt/sources.list.d/supralinux-round20.list
sudo apt-get update
...
record_xbel() {
  ...
  xbel="${home}/.qttest/share/recently-used.xbel"
  [[ -f "${xbel}" ]] || xbel=""
...
run_recent_series() {
  ...
  for ((i=1; i<=reps; i++)); do
    ...
    rm -rf "${home}"
    mkdir -p "${home}/.qttest/share" "${home}/.qttest/config"
...
rm -rf "${capture_home}"
mkdir -p "${capture_home}/.qttest/share" "${capture_home}/.qttest/config"
set +e
(
  cd "${OBJ}/autotests"
...
run_kdir() {
  ...
  (
    ...
    dbus-run-session -- xvfb-run -a -s "-screen 0 1280x1024x24" "${OBJ}/bin/kdirmodeltest" \
      testShowRoot testShowRootWithTrailingSlash testShowRootAndExpandToUrl
  ) >"${log}" 2>&1
  rc=$?
  set -e
...
STAGE=kdir-visible-path-audit
visible_home="${RUNNER_TEMP:?missing RUNNER_TEMP}/supralinux-kdir-home/sbuild"
python3 - "${visible_home}" "${EVIDENCE}/kdir-visible-path-audit.json" <<'PY'
import json,sys
from pathlib import PurePosixPath,Path
home=sys.argv[1]
parts=[p for p in PurePosixPath(home).parts if p not in {"/",""}]
hidden=[p for p in parts if p.startswith(".")]
result={"home":home,"components":parts,"hidden_components":hidden,"valid":not hidden}
Path(sys.argv[2]).write_text(json.dumps(result,indent=2,sort_keys=True)+"\n")
if hidden:
    raise SystemExit("visible HOME contains hidden path components: "+repr(hidden))
PY

STAGE=kdir-visible
run_kdir visible "${visible_home}"
...
baseline_detail=[x for x in recent_rows if x["label"]=="baseline"]
delayed_detail=[x for x in recent_rows if x["label"]=="delayed"]
...
baseline_env_valid=len(baseline_detail)==30 and all(len(x["entries"])==3 for x in baseline_detail)
delayed_env_valid=len(delayed_detail)==30 and all(len(x["entries"])==3 for x in delayed_detail)
capture_env_valid=len(capture["entries"])==15
recent_env_valid=baseline_env_valid and delayed_env_valid and capture_env_valid
capture_dupes=capture["duplicate_modified_groups"]

if not recent_env_valid:
    recent_conclusion="DIAG_INVALID-krecent-environment"
elif baseline_fail>0 and capture_dupes>0 and delayed_fail==0:
    recent_conclusion="krecentdocument-timestamp-collision-ordering-confirmed"
elif baseline_fail==0:
    recent_conclusion="krecentdocument-historical-failure-not-reproduced"
else:
    recent_conclusion="krecentdocument-timestamp-hypothesis-not-confirmed"
...
path_audit=json.loads((ev/"kdir-visible-path-audit.json").read_text())
def historical_fail(x):
    return x["show_root_fail"] and x["show_root_expand_fail"] and x["trailing_slash_pass"]
hidden_a=historical_fail(kdir["hidden-initial"])
hidden_c=historical_fail(kdir["hidden-repeat"])
vis=kdir["visible"]
visible_pass=(not vis["show_root_fail"] and not vis["show_root_expand_fail"] and vis["trailing_slash_pass"] and vis["rc"]==0)
if not path_audit.get("valid") or path_audit.get("hidden_components"):
    kdir_conclusion="DIAG_INVALID-visible-home-path"
elif hidden_a and hidden_c and visible_pass:
    kdir_conclusion="hidden-home-component-controls-kdirmodel-showroot-failure"
elif hidden_a and hidden_c and historical_fail(vis):
    kdir_conclusion="hidden-home-component-rejected-deeper-expand-state-required"
else:
    kdir_conclusion="kdirmodel-baseline-or-control-not-stable"

result={
 "schema":1,"node":"kio","round":20,"diagnostic_result":"DIAG_COMPLETE",
 "claim":"non-promoting-kio-environment-corrected-residual-test-diagnostic",
 "package_attempted":False,"package_state_effect":"none",
 "canonical_source_modified":False,"test_suppression":False,
 "started_at":started,
 "recent":{
   "environment_valid":recent_env_valid,
   "baseline_environment_valid":baseline_env_valid,
   "delayed_environment_valid":delayed_env_valid,
   "capture_all_environment_valid":capture_env_valid,
 ...
 "kdirmodel":{
   "visible_path_audit":path_audit,
   "modes":kdir,
   "conclusion":kdir_conclusion
 },
 "next_scope":"remediation-definition-only-if-causes-confirmed-otherwise-narrower-diagnostic"
}
