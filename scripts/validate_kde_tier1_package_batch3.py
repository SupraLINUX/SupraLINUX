...
for n,e in EXPECTED.items():
 ...
 req(tier[n]['state']=='pending' and tier[n]['packaging']=={'state':'pending'},f'{n}: canonical state must remain pending before build')
 dag_node=dag.get(n)
 req(dag_node is None or dag_node.get('state')=='pending',f'{n}: DAG node must be absent or pending before first package attempt')
