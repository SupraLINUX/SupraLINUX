...
WORK="${ROOT}/.work/kde-tier2-package-batch5/${NODE}"
OUT="${WORK}/out"
EVIDENCE="${ROOT}/evidence/kde-tier2-package-batch5/${NODE}"
...
env={
 ...
 "EXTRA_ABI_CONTRACTS_JSON":json.dumps(n.get("extra_abi_contracts",[]),separators=(",",":")),
}
...
STAGE=extra-abi-contracts
python3 - "${EVIDENCE}/built-debs.json" "${EXTRA_ABI_CONTRACTS_JSON}" "${WORK}" "${EVIDENCE}" <<'PY'
import json,subprocess,sys,shutil
from pathlib import Path
built=json.load(open(sys.argv[1]))
contracts=json.loads(sys.argv[2])
work=Path(sys.argv[3]); out=Path(sys.argv[4])
results=[]
for index,item in enumerate(contracts):
    package=item["package"]; soname=item["soname"]
    if package not in built:
        raise SystemExit(f"extra ABI package not built: {package}")
    root=work/f"extra-abi-{index}"
...
STAGE=legacy-abi-metadata
python3 - "${EVIDENCE}/built-debs.json" "${EVIDENCE}/legacy-abi-metadata.json" <<'PY'
import json,re,subprocess,sys
from pathlib import Path
built=json.load(open(sys.argv[1])); out=Path(sys.argv[2])
legacy=re.compile(r'\\b(?:libkpim6mime6|libkmime-dev|libkmime-data)\\b')
rows={}
for package,path in sorted(built.items()):
    fields={}
    for field in ("Provides","Replaces","Breaks"):
        p=subprocess.run(["dpkg-deb","-f",path,field],text=True,capture_output=True)
        value=p.stdout.strip()
        fields[field]=value
        if legacy.search(value):
            raise SystemExit(f"{package}: forbidden legacy ABI metadata in {field}: {value}")
    rows[package]=fields
out.write_text(json.dumps(rows,indent=2)+"\n")
print("legacy ABI metadata gate: PASS")
PY

STAGE=legacy-runtime-coinstallation
sudo DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends \
  libkpim6mime6 libkmime-data |& tee "${EVIDENCE}/legacy-coinstallation.log"
sudo apt-get check |& tee -a "${EVIDENCE}/legacy-coinstallation.log"
for package_name in libkf6mime-data libkf6mime-dev libkf6mime6; do
  [[ "$(dpkg-query -W -f='${Version}' "${package_name}")" == "${PACKAGE_VERSION}" ]]
done
dpkg-query -W -f='${Package}\t${Version}\n' libkpim6mime6 libkmime-data | tee -a "${EVIDENCE}/legacy-coinstallation.log"
if dpkg-query -W -f='${Status}\n' libkmime-dev 2>/dev/null | grep -q 'install ok installed'; then
  echo "Legacy development package libkmime-dev must not be installed by the runtime compatibility gate" >&2
  exit 1
fi
dpkg -L libkf6mime6 | grep -E '/libKF6Mime\.so\.6($|\.)' | tee "${EVIDENCE}/framework-runtime-paths.txt"
dpkg -L libkpim6mime6 | grep -E '/libKPim6Mime\.so\.6($|\.)' | tee "${EVIDENCE}/legacy-runtime-paths.txt"
echo "legacy runtime co-installation: PASS" | tee -a "${EVIDENCE}/legacy-coinstallation.log"
...
if [[ "${QML_PACKAGES_JSON}" != "[]" ]]; then
  ...
  python3 - "${EVIDENCE}/built-debs.json" "${QML_PACKAGES_JSON}" "${WORK}" "${EVIDENCE}/qml-qmldir.txt" <<'PY'
import json,subprocess,sys,shutil
from pathlib import Path
built=json.load(open(sys.argv[1])); packages=json.loads(sys.argv[2]); work=Path(sys.argv[3]); output=Path(sys.argv[4])
rows=[]
for index,package in enumerate(packages):
    if package not in built:
        raise SystemExit(f"declared QML package not built: {package}")
    root=work/f"qml-root-{index}"
    if root.exists(): shutil.rmtree(root)
    root.mkdir(parents=True)
    subprocess.check_call(["dpkg-deb","-x",built[package],str(root)])
    qmldirs=sorted(root.rglob("qmldir"))
    if not qmldirs:
        raise SystemExit(f"{package}: no qmldir payload")
    rows.extend(f"{package}|{path}" for path in qmldirs)
output.write_text("\n".join(rows)+"\n")
print(f"QML payload smoke PASS: {packages}")
PY
fi
...
if (ev/"extra-abi-contracts.json").exists():
    data["extra_abi_contracts"]="PASS"
    data["extra_abi_evidence"]=json.loads((ev/"extra-abi-contracts.json").read_text())
if (ev/"legacy-abi-metadata.json").exists():
    data["legacy_abi_metadata"]="PASS"
if (ev/"legacy-coinstallation.log").exists():
    data["legacy_runtime_coinstallation"]="PASS"
result_path.write_text(json.dumps(data,indent=2)+"\n")
PY

STATE=PASS
STAGE=complete
echo "KDE Tier 2 Batch 5 ${NODE}: PASS"
