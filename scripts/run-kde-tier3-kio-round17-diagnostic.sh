...
WORK="${ROOT}/.work/kde-tier3-kio-round17-diagnostic"
INPUTS="${WORK}/inputs"
APT_REPO="/tmp/supralinux-round17-repo"
EVIDENCE="${ROOT}/evidence/kde-tier3-kio-round17-diagnostic"
...
Path(p).write_text(json.dumps({
 "schema":1,"node":"kio","round":17,"diagnostic_result":result,
 "exit_code":int(rc),"stage":stage,"started_at":started,"finished_at":finished,
 "claim":"non-promoting-actual-kio-object-path-state-transition-diagnostic",
...
STAGE=contract
python3 scripts/validate_kde_tier3_kio_round17_diagnostic.py
...
echo "deb [trusted=yes] file:${APT_REPO} ./" | sudo tee /etc/apt/sources.list.d/supralinux-round17.list
sudo apt-get update
...
source "${ROOT}/scripts/run-kde-tier3-kio-round17-classify.sh"
