...
if not isinstance(node,dict): raise SystemExit(f'Unknown campaign node: {sys.argv[2]}')
shared=data['shared_predecessors']; signing=data['shared_packaging_inputs']['signing_key']; symbols=node['symbols']
vals={
'UPSTREAM_VERSION':node['upstream_version'],'SOURCE_PACKAGE':node['source_package'],'DEBIAN_VERSION':node['package_version'],
'UPSTREAM_URL':node['source_url'],'UPSTREAM_SHA256':node['source_sha256'],'SYMBOLS_FILE':symbols['file'],
'SYMBOLS_REFERENCE_SHA256':symbols['sha256'],'SYMBOLS_REFERENCE_TREE':symbols['tree_provider'],
'COPYRIGHT_REFERENCE_SHA256':node['copyright']['sha256'],'SIGNING_KEY_SHA256':signing['sha256'],
'RUNTIME_PACKAGE':node['runtime_package'],'SONAME':node['soname'],'CONSUMER_RUN':node['consumer_run'],
'ECM_VERSION':shared['extra_cmake_modules']['version'],'ECM_DEB_SHA256':shared['extra_cmake_modules']['deb_sha256'],
...
WORK_DIR="${ROOT}/.work/kde-tier1-package-batch3/${NODE}"
SOURCE_WORK="${WORK_DIR}/source"
OUT_DIR="${WORK_DIR}/out"
EVIDENCE_DIR="${ROOT}/evidence/kde-tier1-package-batch3/${NODE}"
...
rm -rf "${WORK_DIR}" "${EVIDENCE_DIR}"; mkdir -p "${SOURCE_WORK}" "${OUT_DIR}" "${EVIDENCE_DIR}" "$(dirname "${CHROOT_TARBALL}")"
write_result() {
  local rc="$?" finished_at; finished_at="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
...
. /etc/os-release; [[ "${ID}" == ubuntu && "${VERSION_ID}" == 26.04 ]] || { echo "Expected Ubuntu 26.04" >&2; exit 1; }
sudo apt-get update
sudo DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends binutils ca-certificates cmake curl dbus-daemon devscripts dpkg-dev g++ lintian mmdebstrap ninja-build pkg-config qt6-base-dev qt6-declarative-dev sbuild uidmap ubuntu-keyring xz-utils
...
EXTRACTED="${SOURCE_WORK}/${NODE}-${UPSTREAM_VERSION}"; SOURCE_DIR="${SOURCE_WORK}/${SOURCE_PACKAGE}-${UPSTREAM_VERSION}"
test -d "${EXTRACTED}"; mv "${EXTRACTED}" "${SOURCE_DIR}"
cp -a "${PACKAGE_META}" "${SOURCE_DIR}/debian"; cp -a "${SYMBOLS_REFERENCE}" "${SOURCE_DIR}/debian/${SYMBOLS_FILE}"; cp -a "${COPYRIGHT_REFERENCE}" "${SOURCE_DIR}/debian/copyright"
...
(( ${#DEBS[@]} == ${#EXPECTED_PACKAGES[@]} && ${#CHANGES[@]} == 1 && ${#BUILDINFO[@]} == 1 )) || { echo "Unexpected build artifact counts" >&2; exit 1; }
...
from pathlib import Path
campaign,node_id,out_dir,version=sys.argv[1:]; node=json.loads(Path(campaign).read_text())['nodes'][node_id]; debs={}
for p in Path(out_dir).glob('*.deb'): debs[subprocess.check_output(['dpkg-deb','-f',str(p),'Package'],text=True).strip()]=p
def field(p,n):
...
dev=debs[node['development_package']]; depends=field(dev,'Depends'); rec=field(dev,'Recommends')
if f"{node['runtime_package']} (= {version})" not in depends: raise SystemExit('dev package does not pin runtime')
if 'qt6-base-dev (>= 6.9.0~)' not in depends: raise SystemExit('dev package lost Qt >= 6.9 contract')
if f"{node['documentation_package']} (= {version})" not in rec: raise SystemExit('dev package does not recommend doc package')
PY
...
CONSUMER_BINARY="${CONSUMER_BUILD}/${NODE}-consumer"; MULTIARCH="$(dpkg-architecture -qDEB_HOST_MULTIARCH)"
LD_LIBRARY_PATH="${CONSUMER_ROOT}/usr/lib/${MULTIARCH}${LD_LIBRARY_PATH:+:${LD_LIBRARY_PATH}}" "${CONSUMER_BINARY}" |& tee "${EVIDENCE_DIR}/consumer-run.log"

STAGE="dag-pass-evidence"
{
 echo "node=${NODE}"; echo "state=PASS"; echo "upstream_version=${UPSTREAM_VERSION}"; echo "debian_version=${DEBIAN_VERSION}"; echo "ecm_predecessor=${ECM_VERSION}";
 echo "source_sha256=${UPSTREAM_SHA256}"; echo "symbols_reference_tree=${SYMBOLS_REFERENCE_TREE}"; echo "symbols_baseline_sha256=${SYMBOLS_REFERENCE_SHA256}";
 echo "tests=PASS-via-sbuild"; echo "lintian=PASS-source-and-binary"; echo "abi_soname=${SONAME}"; echo "consumer_smoke=PASS"; echo "downstream_eligible=yes";
} > "${EVIDENCE_DIR}/dag-node.txt"
STATE="PASS"; STAGE="complete"; echo "KDE Tier 1 Batch 3 node ${NODE}: PASS"
