...
for node_id,sha in expected.items():
    ...
    req(planning.get("readiness")=="package-contract-required",f"{node_id}: package-contract-required lifecycle")
...
req(policy.get("phase")=="package-contracts-required","Tier3 discovery phase")
...
req(policy.get("package_contracts")=="required-before-materialization","Tier3 package-contract gate")
...
req(support.get("next_gate")=="tier3-package-contracts","Tier3 support next gate")
...
req("package-contract-required" in doc,"Tier3 docs lifecycle")
...
print("Next gate: Tier 3 package contracts")
