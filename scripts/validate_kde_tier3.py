...
for node_id,sha in expected.items():
    ...
    req(planning.get("provider_audit")=="pending-ci",f"{node_id}: provider audit pending lifecycle")
...
req(policy.get("phase")=="provider-audit-pending","Tier3 discovery phase")
req(policy.get("dependencies")=="materialized-from-kde-upstream-v6.30.0","Tier3 dependency state")
req(policy.get("provider_audit")=="tier3-frameworks-pending-ci","Tier3 provider audit gate")
req(policy.get("package_contracts")=="not-authorized-before-tier3-provider-audit","Tier3 package-contract gate")
req(policy.get("package_builds")=="not-authorized-before-tier3-provider-audit","Tier3 package-build gate")
...
req(support.get("next_gate")=="tier3-provider-audit","Tier3 support next gate")
...
print("Next gate: Tier 3 framework provider audit")
