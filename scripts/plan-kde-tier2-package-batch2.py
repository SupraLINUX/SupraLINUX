...
def main():
    ...
    args=ap.parse_args()
    c=json.loads((ROOT/"manifests/kde-tier2-package-campaign-batch2.json").read_text())
    include=[]
    for node_id in c["selected_nodes"]:
        ...
        include.append({
            "node":node_id,
            "materialization_run":n["materialization"]["workflow_run"],
            "materialization_artifact_id":n["materialization"]["artifact_id"],
            "predecessor_run":n["predecessor"]["workflow_run"],
            "predecessor_artifact_id":n["predecessor"]["artifact_id"],
        })
