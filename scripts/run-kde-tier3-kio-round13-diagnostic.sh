...
INPUTS="${WORK}/inputs"
APT_REPO="/tmp/supralinux-round13-repo"
...
run_direct_pair() {
  ...
  for bin in kdirmodeltest knewfilemenutest; do
    case "${bin}" in
      kdirmodeltest) test_case=testIcon ;;
      knewfilemenutest) test_case=testFolderIconCollection ;;
      *) echo "unexpected test binary: ${bin}" >&2; return 2 ;;
    esac
    set +e
    env HOME="${home}" KDECI_PLATFORM_PATH="${SRC}" \
      QT_QPA_PLATFORM=xcb QT_QPA_SYSTEM_ICON_THEME=breeze QT_PLUGIN_PATH="${OBJ}/bin" \
      "$@" timeout --signal=TERM --kill-after=10s "${TEST_TIMEOUT_SECONDS}s" \
      dbus-run-session -- xvfb-run -a -s '-screen 0 1280x1024x24' "${OBJ}/bin/${bin}" "${test_case}" \
      >> "${EVIDENCE}/${label}.log" 2>&1
    rc=$?
    set -e
    [[ "${rc}" -eq 0 ]] || overall=1
...
for bin in kdirmodeltest knewfilemenutest; do
  ...
  case "${bin}" in
    kdirmodeltest) test_case=testIcon ;;
    knewfilemenutest) test_case=testFolderIconCollection ;;
    *) echo "unexpected trace binary: ${bin}" >&2; exit 2 ;;
  esac
  mkdir -p "${HOME_BASE}/${label}"
  set +e
  HOME="${HOME_BASE}/${label}" KDECI_PLATFORM_PATH="${SRC}" \
  QT_QPA_PLATFORM=xcb QT_QPA_SYSTEM_ICON_THEME=breeze QT_PLUGIN_PATH="${OBJ}/bin" QT_DEBUG_PLUGINS=1 \
  timeout --signal=TERM --kill-after=10s "${TRACE_TIMEOUT_SECONDS}s" \
    dbus-run-session -- xvfb-run -a -s '-screen 0 1280x1024x24' \
    strace -f -e trace=file -o "${EVIDENCE}/${label}.strace" "${OBJ}/bin/${bin}" "${test_case}" \
    > "${EVIDENCE}/${label}.log" 2>&1
  echo "$?" > "${EVIDENCE}/${label}.rc"
  set -e
done
