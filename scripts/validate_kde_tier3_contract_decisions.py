...
if active:
    ...
    req(active.get("status") in {"materialization-pending-ci","materialization-PASS","materialization-PASS-attempt3-active","materialization-PASS-attempt4-active","materialization-PASS-attempt5-active","materialization-PASS-attempt6-active","provider-closure-pending-attempt2-activation-validation","provider-closure-PASS"},"Tier3 active remediation state")
    if round_no==10:
        req(active_nodes=={"kio","kxmlgui"},"Tier3 round10 Level1 failure set")
        req(active.get("level")=="build-level1","Tier3 round10 Level1 identity")
        req(active.get("trigger",{}).get("workflow_run")==35991007820 and active.get("trigger",{}).get("commit")=="3197c1988e1ab86ec5010cb693064db949bfe6b0","Tier3 round10 trigger")
        req(active.get("trigger",{}).get("level1_result")=="0 workflow SUCCESS / 2 FAIL","Tier3 round10 Attempt5 result")
        req(active.get("source_changed_nodes")==["kio","kxmlgui"] and active.get("provider_closure_only_nodes")==[],"Tier3 round10 source-remediation classes")
        req(active.get("candidate_package_versions")=={"kio":"6.30.0-0supralinux6","kxmlgui":"6.30.0-0supralinux5"},"Tier3 round10 package revisions")
        if active.get("status") in {"materialization-PASS","materialization-PASS-attempt6-active"}:
            ev2=active.get("evidence",{})
            req(ev2.get("workflow_run")==36002910277 and ev2.get("commit")=="c0774e5514fd83995aad3c86e1f6a5b106b001a3","Tier3 round10 materialization evidence")
            req(ev2.get("artifacts",{}).get("kio",{}).get("artifact_id")==10809231495 and ev2.get("artifacts",{}).get("kxmlgui",{}).get("artifact_id")==10808294092,"Tier3 round10 materialization artifacts")
            if active.get("status")=="materialization-PASS-attempt6-active":
                req(active.get("next_gate")=="tier3-build-level1-attempt6","Tier3 round10 Attempt6 contract gate")
                req(active.get("activation_policy_workflow_run")==36032425645 and active.get("activation_level1_workflow_run")==36032425729,"Tier3 round10 Attempt6 validation evidence")
                req(active.get("activation_commit")=="0fee159ac1f24dc160e1edd9976a7708f89d657d","Tier3 round10 Attempt6 validation commit")
            else:
                req(active.get("next_gate")=="tier3-build-level1-planning-validation","Tier3 round10 promoted next gate")
        else:
            req(active.get("status")=="materialization-pending-ci" and active.get("next_gate")=="tier3-round10-level1-materialization","Tier3 round10 pending materialization gate")
        pol=active.get("policy",{})
        req(pol.get("package_revision_bump_required_for_packaging_changes") is True and pol.get("rematerialize_only_changed_nodes") is True and pol.get("kde_dependency_graph_unchanged") is True,"Tier3 round10 remediation policy")
        ev=active.get("failure_evidence",{}); root=ev.get("rootfs",{})
        req(root.get("artifact_id")==10804620493 and root.get("artifact_sha256")=="0855a64addfe52eb38ae3f71b4e7a8c3c1a08c5351dfa677ba03e0933426ca21" and root.get("rootfs_sha256")=="ecc2df763b7dd14b8812bfebedecb1ff3c532dd970cb3f1f5b2973b750d9740c","Tier3 round10 rootfs evidence")
        req(ev.get("kio",{}).get("job_id")==107605066803 and ev.get("kio",{}).get("artifact_id")==10804966399 and ev.get("kio",{}).get("artifact_sha256")=="850d56de4b07a9b0d2fd3dd2730ef13381fdcf4f0e9059e097cb829809a25ad3" and ev.get("kio",{}).get("failure_substage")=="ctest/per-test-offscreen-icon-theme","Tier3 round10 KIO failure evidence")
        req(ev.get("kxmlgui",{}).get("job_id")==107605066954 and ev.get("kxmlgui",{}).get("artifact_id")==10804084102 and ev.get("kxmlgui",{}).get("artifact_sha256")=="92adc73ad8242e07299b5ae20c0acd100979e78f0b7da6ebb8fb5686c4c7204a" and ev.get("kxmlgui",{}).get("failure_substage")=="post-build/buildinfo-and-symbols-policy","Tier3 round10 KXMLGui failure evidence")
        hist9=[x for x in c.get("remediation_history",[]) if x.get("round")==9]
        req(len(hist9)==1 and hist9[0].get("status")=="attempt5-FAIL" and hist9[0].get("validation",{}).get("workflow_run")==35991007820 and hist9[0].get("validation",{}).get("canonical_promotions")==0,"Tier3 round9 contract history retains Attempt5 result")
    elif round_no==9:
        ...
        if active.get("status") in {"materialization-PASS","materialization-PASS-attempt5-active"}:
            ev2=active.get("evidence",{})
            req(ev2.get("workflow_run")==35965579279 and ev2.get("commit")=="8380856c8161dccc9de9c12745012c555baa26b0","Tier3 round9 materialization evidence")
            req(ev2.get("artifacts",{}).get("kio",{}).get("artifact_id")==10794251210 and ev2.get("artifacts",{}).get("kxmlgui",{}).get("artifact_id")==10793229286,"Tier3 round9 materialization artifacts")
            if active.get("status")=="materialization-PASS-attempt5-active":
                req(active.get("next_gate")=="tier3-build-level1-attempt5","Tier3 round9 Attempt5 contract gate")
                req(active.get("activation_policy_workflow_run")==35990068378 and active.get("activation_level1_workflow_run")==35990068382,"Tier3 round9 Attempt5 validation evidence")
                req(active.get("activation_commit")=="45675c1fb5a43f95204c2cf0c5c15df0ef703832","Tier3 round9 Attempt5 validation commit")
            else:
                req(active.get("next_gate")=="tier3-build-level1-planning-validation","Tier3 round9 promoted next gate")
        else:
        ...
        if active.get("status") in {"materialization-PASS","materialization-PASS-attempt4-active"}:
            ev2=active.get("evidence",{})
            ...
            if active.get("status")=="materialization-PASS-attempt4-active":
                req(active.get("next_gate")=="tier3-build-level1-attempt4","Tier3 round8 Attempt4 contract gate")
                req(active.get("activation_policy_workflow_run")==35895610944 and active.get("activation_level1_workflow_run")==35895610937,"Tier3 round8 Attempt4 validation evidence")
                req(active.get("activation_commit")=="9f680b0d8790c7bc472e8352e6675d606f706ee7","Tier3 round8 Attempt4 validation commit")
            else:
...
for node_id in selected:
    ...
    expected_versions={
        ...
        "kio":"6.30.0-0supralinux6" if active.get("round",0)>=10 else ("6.30.0-0supralinux5" if active.get("round",0)>=9 else ("6.30.0-0supralinux4" if active.get("round",0)>=8 else ("6.30.0-0supralinux3" if active.get("round",0)>=7 else ("6.30.0-0supralinux2" if active.get("round",0)>=5 else "6.30.0-0supralinux1")))),
        "kxmlgui":"6.30.0-0supralinux5" if active.get("round",0)>=10 else ("6.30.0-0supralinux4" if active.get("round",0)>=9 else ("6.30.0-0supralinux3" if active.get("round",0)>=8 else ("6.30.0-0supralinux2" if active.get("round",0)>=7 else "6.30.0-0supralinux1"))),
    }
...
if active.get("round")==10:
    rel=[(x.get("action"),x.get("package") or x.get("relation")) for x in kio.get("source_build_relation_overrides",[])]
    req(rel==[("remove","libkf6auth-dev"),("remove","libkf6configwidgets-dev"),("ensure","dbus-daemon <!nocheck>"),("ensure","breeze-icon-theme (>= 4:6.30.0~) <!nocheck>"),("ensure","xvfb <!nocheck>")],"KIO round10 retained providers")
    rr=kio.get("rules_text_replacements",[])
    req(len(rr)==3,"KIO round10 rules delta count")
    targeted=[x for x in rr if x.get("classification")=="upstream-test-environment-targeted-correction"]
    req(len(targeted)==1 and "kiowidgets-kdirmodeltest" in targeted[0].get("new","") and "kiofilewidgets-knewfilemenutest" in targeted[0].get("new","") and "QT_QPA_PLATFORM=xcb;QT_QPA_SYSTEM_ICON_THEME=breeze" in targeted[0].get("new",""),"KIO round10 targeted CTest QPA correction")
    ...
    req(env.get("upstream_per_test_environment_preserved_for_other_tests") is True and env.get("external_network",{}).get("required") is True and env.get("ctest_parallelism")==1,"KIO round10 retained environment")
    ...
    req(xrel==[("ensure","python3-build"),("ensure","python3-setuptools"),("ensure","dbus-daemon <!nocheck>"),("ensure","libkf6textwidgets-dev (>= 6.30.0~) <!nocheck>")],"KXMLGui round10 exact build/test providers")
    adds=xnode.get("symbol_template_additions",[])
    req(len(adds)==1 and adds[0].get("package")=="libkf6xmlgui6" and adds[0].get("soname")=="libKF6XmlGui.so.6" and adds[0].get("symbol")=="_ZSt19piecewise_construct@Base" and adds[0].get("minimal_version")=="6.30.0" and adds[0].get("tags")==["optional"],"KXMLGui round10 optional template-symbol baseline")
    xr=xnode.get("rules_text_replacements",[])
    req(len(xr)==1 and xr[0].get("new")=="override_dh_auto_test:\n\tdbus-run-session -- env QT_QPA_PLATFORM=offscreen dh_auto_test","KXMLGui round10 retained 7/7 D-Bus offscreen suite")
    req(any(q.get("decision")=="declare-kxmlgui-ktextwidgets-test-build-dependency" for q in xnode.get("contract_decisions",[])) and any(q.get("decision")=="add-kxmlgui-optional-piecewise-template-symbol-baseline" for q in xnode.get("contract_decisions",[])),"KXMLGui round10 decisions documented")
elif active.get("round")==9:
    rel=[(x.get("action"),x.get("package") or x.get("relation")) for x in kio.get("source_build_relation_overrides",[])]
...
if active:
    ...
    if active.get("round")==10:
        req(canonical.get("phase") in {"build-level1-planning","build-level1"} and canonical.get("package_builds") in {"tier3-level1-remediation-pending-materialization","tier3-level1-source-PASS-pending-planning-validation","tier3-level1-authorized"},"Tier3 round10 canonical materialization/planning/build gate")
        req(ar.get("level")=="build-level1" and ar.get("status") in {"materialization-pending-ci","materialization-PASS-pending-level1-planning-validation","level1-active-pending-ci"},"Tier3 round10 canonical Level1 state")
        req(set(ar.get("nodes",[]))=={"kio","kxmlgui"} and ar.get("source_materialization_nodes")==["kio","kxmlgui"] and ar.get("provider_closure_only_nodes")==[],"Tier3 round10 canonical source scope")
        req(ar.get("candidate_package_versions")=={"kio":"6.30.0-0supralinux6","kxmlgui":"6.30.0-0supralinux5"},"Tier3 round10 canonical revisions")
        req(ar.get("validation_workflow_run")==35991007820 and ar.get("validation_commit")=="3197c1988e1ab86ec5010cb693064db949bfe6b0","Tier3 round10 canonical Attempt5 evidence")
        req(ar.get("validation_result")=="0 workflow SUCCESS / 2 FAIL" and set(ar.get("remaining_failed_nodes",[]))=={"kio","kxmlgui"} and ar.get("canonical_promotions")==0,"Tier3 round10 canonical failure/no-promotion semantics")
        if canonical.get("package_builds")=="tier3-level1-authorized":
            req(ar.get("status")=="level1-active-pending-ci" and ar.get("execution_authorized") is True and ar.get("level1_execution_authorized") is True,"Tier3 round10 Attempt6 canonical active state")
            req(ar.get("current_attempt")==6 and ar.get("activation_policy_workflow_run")==36032425645 and ar.get("activation_level1_workflow_run")==36032425729,"Tier3 round10 Attempt6 canonical validation")
            req(ar.get("activation_commit")=="0fee159ac1f24dc160e1edd9976a7708f89d657d" and ar.get("next_gate")=="tier3-build-level1-attempt6","Tier3 round10 Attempt6 canonical gate")
        elif canonical.get("package_builds")=="tier3-level1-source-PASS-pending-planning-validation":
            req(ar.get("execution_authorized") is False and ar.get("level1_execution_authorized") is False and ar.get("current_attempt")==5 and ar.get("next_attempt")==6,"Tier3 round10 canonical execution pause")
            req(ar.get("status")=="materialization-PASS-pending-level1-planning-validation","Tier3 round10 canonical source PASS state")
            req(ar.get("materialization_workflow_run")==36002910277 and ar.get("materialization_commit")=="c0774e5514fd83995aad3c86e1f6a5b106b001a3","Tier3 round10 canonical source PASS evidence")
            req(ar.get("next_gate")=="tier3-build-level1-planning-validation","Tier3 round10 canonical planning gate")
        else:
            req(ar.get("execution_authorized") is False and ar.get("level1_execution_authorized") is False and ar.get("current_attempt")==5 and ar.get("next_attempt")==6,"Tier3 round10 canonical execution pause")
            req(ar.get("status")=="materialization-pending-ci" and ar.get("next_gate")=="tier3-round10-level1-materialization","Tier3 round10 canonical materialization gate")
    elif active.get("round")==9:
        req(canonical.get("phase") in {"build-level1-planning","build-level1"} and canonical.get("package_builds") in {"tier3-level1-remediation-pending-materialization","tier3-level1-source-PASS-pending-planning-validation","tier3-level1-authorized"},"Tier3 round9 canonical materialization/planning/build gate")
        req(ar.get("level")=="build-level1" and ar.get("status") in {"materialization-pending-ci","materialization-PASS-pending-level1-planning-validation","level1-active-pending-ci"},"Tier3 round9 canonical Level1 state")
        ...
        if canonical.get("package_builds")=="tier3-level1-authorized":
            req(ar.get("status")=="level1-active-pending-ci" and ar.get("execution_authorized") is True and ar.get("level1_execution_authorized") is True,"Tier3 round9 Attempt5 canonical active state")
            req(ar.get("current_attempt")==5 and ar.get("activation_policy_workflow_run")==35990068378 and ar.get("activation_level1_workflow_run")==35990068382,"Tier3 round9 Attempt5 canonical validation")
            req(ar.get("activation_commit")=="45675c1fb5a43f95204c2cf0c5c15df0ef703832" and ar.get("next_gate")=="tier3-build-level1-attempt5","Tier3 round9 Attempt5 canonical gate")
        elif canonical.get("package_builds")=="tier3-level1-source-PASS-pending-planning-validation":
            ...
            req(ar.get("status")=="materialization-PASS-pending-level1-planning-validation","Tier3 round9 canonical source PASS state")
            req(ar.get("materialization_workflow_run")==35965579279 and ar.get("materialization_commit")=="8380856c8161dccc9de9c12745012c555baa26b0","Tier3 round9 canonical source PASS evidence")
            req(ar.get("next_gate")=="tier3-build-level1-planning-validation","Tier3 round9 canonical planning gate")
        else:
            req(ar.get("execution_authorized") is False and ar.get("level1_execution_authorized") is False and ar.get("current_attempt")==4 and ar.get("next_attempt")==5,"Tier3 round9 canonical execution pause")
            req(ar.get("status")=="materialization-pending-ci" and ar.get("next_gate")=="tier3-round9-level1-materialization","Tier3 round9 canonical materialization gate")
    elif active.get("round")==8:
        ...
        if canonical.get("package_builds")=="tier3-level1-authorized":
            req(ar.get("status")=="level1-active-pending-ci" and ar.get("execution_authorized") is True and ar.get("level1_execution_authorized") is True,"Tier3 round8 Attempt4 canonical active state")
            req(ar.get("current_attempt")==4 and ar.get("activation_policy_workflow_run")==35895610944 and ar.get("activation_level1_workflow_run")==35895610937,"Tier3 round8 Attempt4 canonical validation")
            req(ar.get("activation_commit")=="9f680b0d8790c7bc472e8352e6675d606f706ee7" and ar.get("next_gate")=="tier3-build-level1-attempt4","Tier3 round8 Attempt4 canonical gate")
        elif canonical.get("package_builds")=="tier3-level1-source-PASS-pending-planning-validation":
