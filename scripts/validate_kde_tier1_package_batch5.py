...
req(c.get('state')=='prepared-pending-build','open batch state')
...
for node,e in EXPECTED.items():
    n=c['nodes'][node]
    req(n['state']=='prepared-pending-build',f'{node}: preparation state')
    req(n['last_result'] is None and n['evidence']==[],f'{node}: no fabricated attempt evidence')
    req(n['downstream_eligible'] is False,f'{node}: not downstream eligible before build')
    req(n['package_version']=='6.30.0-0supralinux1',f'{node}: initial revision')
    ...
    req(changelog.startswith(f"{n['source_package']} (6.30.0-0supralinux1) resolute;"),f'{node}: changelog revision')
...
req('dbus-run-session dh_auto_test -Skf6' in text(ROOT/'packages/kde/modemmanager-qt/debian/rules'),'ModemManagerQt D-Bus test session')
...
req('prepared; package attempts pending' in text(DOC),'Batch 5 doc must not claim PASS')
...
if errors:
    ...
    raise SystemExit(1)
print('KDE Tier 1 Batch 5 preparation validation: PASS')
