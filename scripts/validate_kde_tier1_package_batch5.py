...
req(sum(1 for n in tier.values() if n.get("state") == "PASS") == 29, "Tier1 current PASS count must be 29")
req(sum(1 for n in tier.values() if n.get("state") == "pending") == 0, "Tier1 current pending count must be 0")
...
for path in (DAG_DOC, DEPENDENCY_DOC):
    t = text(path)
    req("29 PASS" in t and "0 pending" in t, f"{path.name}: current canonical count")
