...
req(sum(1 for n in tier.values() if n.get("state") == "PASS") == 22, "Tier1 current PASS count must be 22")
req(sum(1 for n in tier.values() if n.get("state") == "pending") == 7, "Tier1 current pending count must be 7")
...
for path in (DAG_DOC, DEPENDENCY_DOC):
    t = text(path)
    req("22 PASS" in t and "7 pending" in t, f"{path.name}: current canonical count")
...
print("Canonical Tier 1: 22 PASS / 7 pending / 0 FAIL / 0 BLOCKED")
