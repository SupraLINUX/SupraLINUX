...
case "${NODE}" in
    ktexttemplate|karchive|kholidays) ;;
    *)
...
if printf '%s\n' "${changed_paths[@]}" | grep -Fxq 'manifests/kde-tier1-package-campaign-batch2.json'; then
    campaign_changed=true
    if ! git cat-file -e "${BEFORE}:manifests/kde-tier1-package-campaign-batch2.json" 2>/dev/null; then
...
def load(ref):
    return json.loads(subprocess.check_output(
        ["git", "show", f"{ref}:manifests/kde-tier1-package-campaign-batch2.json"],
        text=True,
    ))
...
if printf '%s\n' "${changed_paths[@]}" | grep -Fxq 'scripts/kde-tier1-package-batch2-needed.sh'; then
    state="$(python3 - "${AFTER}" "${NODE}" <<'PY'
import json, subprocess, sys
ref, node = sys.argv[1:]
data = json.loads(subprocess.check_output(
    ["git", "show", f"{ref}:manifests/kde-tier1-package-campaign-batch2.json"], text=True))
