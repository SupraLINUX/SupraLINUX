...
case "${NODE}" in kitemviews|kglobalaccel|syntax-highlighting) ;; *) echo "Unsupported Batch 4 node: ${NODE}" >&2; exit 2;; esac
...
for p in "${changed[@]}"; do
 case "$p" in
  packages/kde/"${NODE}"/*|scripts/run-kde-tier1-package-batch4-preflight.sh|.github/workflows/kde-tier1-package-batch4.yml) exit 0;;
 esac
done
if printf '%s\n' "${changed[@]}" | grep -Fxq manifests/kde-tier1-package-campaign-batch4.json; then
 if ! git cat-file -e "${BEFORE}:manifests/kde-tier1-package-campaign-batch4.json" 2>/dev/null; then exit 0; fi
 if ! python3 - "${BEFORE}" "${AFTER}" "${NODE}" <<'PY'
import json,subprocess,sys
b,a,n=sys.argv[1:]
def load(r): return json.loads(subprocess.check_output(['git','show',f'{r}:manifests/kde-tier1-package-campaign-batch4.json'],text=True))
def fp(d):
