...
WORK_DIR="${ROOT}/.work/kde-tier1-package-batch5/${NODE}"
SOURCE_WORK="${WORK_DIR}/source"
OUT_DIR="${WORK_DIR}/out"
EVIDENCE_DIR="${ROOT}/evidence/kde-tier1-package-batch5/${NODE}"
...
echo "=== SupraLINUX KDE Tier 1 Batch 5: ${NODE} ${UPSTREAM_VERSION} (${DEBIAN_VERSION}) ==="
...
sudo apt-get update
sudo DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends binutils ca-certificates cmake curl devscripts dpkg-dev g++ libglib2.0-dev libnm-dev lintian mmdebstrap modemmanager-dev ninja-build pkg-config qt6-base-dev qt6-declarative-dev sbuild uidmap ubuntu-keyring xz-utils
...
{ cat /etc/os-release; uname -a; sbuild --version; mmdebstrap --version; lintian --version; pkg-config --modversion ModemManager; pkg-config --modversion libnm; } > "${EVIDENCE_DIR}/host.txt"
...
STATE="PASS"; STAGE="complete"; echo "KDE Tier 1 Batch 5 node ${NODE}: PASS"
