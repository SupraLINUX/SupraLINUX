...
counts={s:sum(x.get('state')==s for x in t['nodes']) for s in ('PASS','pending','FAIL','BLOCKED')}; req(counts=={'PASS':27,'pending':2,'FAIL':0,'BLOCKED':0},f'Tier1 current counts {counts}')
...
print('Canonical Tier 1 current state: 27 PASS / 2 pending / 0 current FAIL / 0 BLOCKED; Batch 6 historical closure remains 18/11')
