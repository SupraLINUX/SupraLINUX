...
req(kw['package_version']=='6.30.0-0supralinux3','kwindowsystem: revision -3')
req(len(kw.get('evidence',[]))==2 and [x.get('result') for x in kw['evidence']]==['FAIL','FAIL'],'kwindowsystem: two retained FAIL attempts')
kev1,kev2=kw['evidence']
...
req(kw.get('remediation',{}).get('candidate_package_version')=='6.30.0-0supralinux3','kwindowsystem: remediation revision -3')
req('OpenBox' in kw.get('remediation',{}).get('upstream_test_reference',''),'kwindowsystem: upstream fixture evidence')
...
req('KWindowSystem' in txt(DOC) and 'Solid' in txt(DOC) and 'third build pending' in txt(DOC).lower(),'Batch6 documentation'); req('multi-library' in txt(DOC) and 'Python bindings' in txt(DOC),'deferral rationale')
if errs:
 for e in errs: print('ERROR:',e,file=sys.stderr)
 raise SystemExit(1)
print('KDE Tier 1 Batch 6 third-attempt preparation validation: PASS')
