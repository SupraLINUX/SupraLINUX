...
counts={s:sum(x.get('state')==s for x in t['nodes']) for s in ('PASS','pending','FAIL','BLOCKED')}; req(counts=={'PASS':29,'pending':0,'FAIL':0,'BLOCKED':0},f'Tier1 current counts {counts}')
...
print('Canonical Tier 1 current state: 29 PASS / 0 pending / 0 current FAIL / 0 BLOCKED; Batch 6 historical closure remains 18/11')
