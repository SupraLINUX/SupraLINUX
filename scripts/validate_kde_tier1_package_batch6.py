...
req(c.get('schema')==1,'schema'); req(c.get('authority')=='kde-upstream','authority'); req(c.get('frameworks_series')=='6.30.0','series'); req(c.get('batch')=='tier1-batch-6','batch'); req(c.get('state')=='prepared-pending-build','batch prepared state'); req(c.get('selected_nodes')==list(E),'selected nodes')
...
for node,e in E.items():
 n=c['nodes'][node]; req(n['state']=='prepared-pending-build' and n['last_result'] is None and n['downstream_eligible'] is False,f'{node}: prepared state'); req(n['package_version']=='6.30.0-0supralinux1',f'{node}: revision'); req(n['source_sha256']==e['source'],f'{node}: source'); req(n['root_cmake_blob']==e['blob'],f'{node}: CMake blob'); req(n['symbols']['sha256']==e['symbols'],f'{node}: symbols'); req(n['copyright']['sha256']==e['copyright'],f'{node}: copyright'); req(n['signing_key']['sha256']==e['key'],f'{node}: key hash'); req(n['signing_key']['signer_fingerprint']=='90A968ACA84537CC27B99EAF2C8DF587A6D4AAC1',f'{node}: signer'); req(n['runtime_package']==e['runtime'] and n['soname']==e['soname'],f'{node}: ABI'); req(n['cmake_package']==e['cmake'] and n['cmake_target']==e['target'],f'{node}: CMake consumer'); req([x['name'] for x in n['binary_contracts']]==e['packages'],f'{node}: package list'); req(n.get('evidence')==[],f'{node}: no invented evidence')
...
if errs:
 ...
 raise SystemExit(1)
print('KDE Tier 1 Batch 6 preparation validation: PASS')
