...
case "${NODE}" in kidletime|modemmanager-qt|networkmanager-qt) ;; *) echo "Unsupported Batch 5 node: ${NODE}" >&2; exit 2;; esac
...
for p in "${changed[@]}"; do
 case "$p" in
  packages/kde/"${NODE}"/*|scripts/run-kde-tier1-package-batch5-preflight.sh|.github/workflows/kde-tier1-package-batch5.yml) exit 0;;
 esac
done
if printf '%s\n' "${changed[@]}" | grep -Fxq manifests/kde-tier1-package-campaign-batch5.json; then
 if ! git cat-file -e "${BEFORE}:manifests/kde-tier1-package-campaign-batch5.json" 2>/dev/null; then exit 0; fi
 if ! python3 - "${BEFORE}" "${AFTER}" "${NODE}" <<'PY2'
import json,subprocess,sys
b,a,n=sys.argv[1:]
def load(r): return json.loads(subprocess.check_output(['git','show',f'{r}:manifests/kde-tier1-package-campaign-batch5.json'],text=True))
def fp(d):
...
if printf '%s\n' "${changed[@]}" | grep -Fxq scripts/kde-tier1-package-batch5-needed.sh; then
 state="$(python3 - "${AFTER}" "${NODE}" <<'PY2'
import json,subprocess,sys
d=json.loads(subprocess.check_output(['git','show',f'{sys.argv[1]}:manifests/kde-tier1-package-campaign-batch5.json'],text=True)); print(d['nodes'][sys.argv[2]]['state'])
PY2
)"
 [[ "${state}" == PASS ]] || exit 0
fi
