...
if audit.get("status")=="pending-ci":
    ...
    for node,c in components.items():
        ...
        req(dep.get("readiness") in {"package-contract-required","package-contract-ready","materialized","build-level1-ready","PASS"},f"{node}: support package-contract lifecycle")
