...
# disabled for this second boot so C4.1a owns the serial acceptance channel.
...
# Persistence is certified across a real SDDM/Plasma session restart. This is a
# CI-only fixture override; it is not a SupraLINUX product setting.
cat >"${MOUNT_DIR}/etc/sddm.conf.d/99-aurora-ci-c4-1a-relogin.conf" <<'SDDM'
[Autologin]
User=auroraci
Relogin=true
SDDM
...
bind_live_session || fail "initial Plasma Wayland session did not become ready"
...
[[ "$(vdm_current)" == "${desktop_id}" ]] || fail "invalid virtual desktop switch changed current desktop"
...
cp "${ci_home}/.config/kwinrc" "${OUT}/kwinrc-mutated" 2>/dev/null || true

stage PERSISTENCE
old_session_id="${session_id}"
old_kwin_pid="${kwin_pid}"
old_plasma_pid="${plasma_pid}"
systemctl restart sddm.service
bind_live_session "${old_session_id}" || fail "Plasma Wayland session did not return after SDDM restart"
[[ "${kwin_pid}" != "${old_kwin_pid}" ]] || fail "KWin PID did not change across intentional session restart"
[[ "${plasma_pid}" != "${old_plasma_pid}" ]] || fail "plasmashell PID did not change across intentional session restart"
activity_present "${activity_id}" || fail "created activity did not persist across session restart"
[[ "$(activity_call ActivityName "${activity_id}")" == "${ACTIVITY_NAME}" ]] || fail "persisted activity name changed"
[[ "$(vdm_count)" == "${mutated_count}" ]] || fail "virtual desktop count did not persist across session restart"
[[ "$(vdm_desktop_id_by_name "${DESKTOP_NAME}")" == "${desktop_id}" ]] || fail "created virtual desktop identity did not persist across session restart"
activity_call ListActivities | sort >"${OUT}/activities-after-relogin.txt"
vdm_dump >"${OUT}/virtual-desktops-after-relogin.txt"
vdm_property count >"${OUT}/virtual-desktop-count-after-relogin.txt"
vdm_property current >"${OUT}/virtual-desktop-current-after-relogin.txt"
cp "${ci_home}/.config/kwinrc" "${OUT}/kwinrc-after-relogin" 2>/dev/null || true

# Re-exercise switching after persistence so the newly started backends consume
# the persisted objects, rather than only proving that configuration files exist.
[[ "$(activity_call SetCurrentActivity "${activity_id}")" == "true" ]] || fail "persisted activity could not be selected after relogin"
wait_until 20 activity_current_is "${activity_id}" || fail "persisted activity did not become current after relogin"
run_user busctl --user set-property \
  org.kde.KWin /VirtualDesktopManager org.kde.KWin.VirtualDesktopManager \
  current s "${desktop_id}"
wait_until 20 vdm_current_is "${desktop_id}" || fail "persisted virtual desktop could not be selected after relogin"
...
for ((i = 0; i < 20; i++)); do
  [[ "$(vdm_count)" == "${baseline_count}" ]] && break
  sleep 1
done
[[ "$(vdm_count)" == "${baseline_count}" ]] || fail "virtual desktop count did not return to baseline"
...
cp "${ci_home}/.config/kwinrc" "${OUT}/kwinrc-final" 2>/dev/null || true
...
[[ "$(activity_call CurrentActivity)" == "${baseline_activity}" ]] || fail "current activity was not restored"
[[ "$(vdm_current)" == "${baseline_desktop}" ]] || fail "current virtual desktop was not restored"
cleanup_done=1
...
[[ "$(first_user_pid kwin_wayland)" == "${kwin_pid}" ]] || fail "KWin restarted unexpectedly after relogin"
[[ "$(first_user_pid plasmashell)" == "${plasma_pid}" ]] || fail "plasmashell restarted unexpectedly after relogin"
...
journalctl -b -u sddm.service --no-pager >"${OUT}/sddm-journal.txt" 2>&1 || true
journalctl -b _UID="${ci_uid}" --no-pager >"${OUT}/user-journal.txt" 2>&1 || true
...
if [[ ${qemu_status} -eq 124 ]]; then
  echo "Aurora C4.1a VM timed out after ${BOOT_TIMEOUT} seconds." >&2
  exit 1
fi
if [[ ${qemu_status} -ne 0 ]]; then
  echo "Aurora C4.1a QEMU exited with status ${qemu_status}." >&2
  exit 1
fi
...
for required_stage in SESSION BASELINE ACTIVITIES VIRTUAL_DESKTOPS PERSISTENCE CLEANUP STABILITY COMPLETE; do
...
for evidence in \
  activities-baseline.txt activities-mutated.txt activities-after-relogin.txt activities-final.txt \
  virtual-desktops-baseline.txt virtual-desktops-mutated.txt virtual-desktops-after-relogin.txt virtual-desktops-final.txt \
