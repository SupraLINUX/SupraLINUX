...
CAMPAIGN="${ROOT}/manifests/kde-tier3-build-level0.json"
: "${ROOTFS_ARTIFACT_DIR:?missing shared Resolute rootfs artifact}"
: "${GITHUB_TOKEN:?missing GitHub Actions token}"

WORK="${ROOT}/.work/kde-tier3-build-level0/${NODE}"
INPUTS="${WORK}/inputs"
OUT="${WORK}/out"
EVIDENCE="${ROOT}/evidence/kde-tier3-build-level0/${NODE}"
...
if c.get("execution_authorized") is not True:
    raise SystemExit("Tier3 Level0 execution is not authorized")
...
STAGE=complete
echo "KDE Tier 3 Level 0 ${NODE}: build PASS; transition=${SUCCESS_TRANSITION}"
