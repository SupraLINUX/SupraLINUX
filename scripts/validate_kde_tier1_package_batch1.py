...
require(sum(1 for item in tier_nodes.values() if item.get("state") == "PASS") == 25, "Canonical Tier 1 PASS count must be 25 after Batch 9 closure")
require(sum(1 for item in tier_nodes.values() if item.get("state") == "pending") == 4, "Canonical Tier 1 pending count must be 4 after Batch 9 closure")
...
print("Canonical Tier 1 now: 25 PASS, 4 pending, 0 FAIL, 0 BLOCKED; Batch 1 historical closure remains 7/22")
