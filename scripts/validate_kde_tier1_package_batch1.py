...
require(sum(1 for item in tier_nodes.values() if item.get("state") == "PASS") == 27, "Canonical Tier 1 PASS count must be 27 after Batch 10 closure")
require(sum(1 for item in tier_nodes.values() if item.get("state") == "pending") == 2, "Canonical Tier 1 pending count must be 2 after Batch 10 closure")
...
print("Canonical Tier 1 now: 27 PASS, 2 pending, 0 FAIL, 0 BLOCKED; Batch 1 historical closure remains 7/22")
