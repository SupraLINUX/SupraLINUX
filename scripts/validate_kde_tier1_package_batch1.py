...
require(sum(1 for item in tier_nodes.values() if item.get("state") == "PASS") == 29, "Canonical Tier 1 PASS count must be 29 after Batch 11 closure")
require(sum(1 for item in tier_nodes.values() if item.get("state") == "pending") == 0, "Canonical Tier 1 pending count must be 0 after Batch 11 closure")
...
print("Canonical Tier 1 now: 29 PASS, 0 pending, 0 FAIL, 0 BLOCKED; Batch 1 historical closure remains 7/22")
