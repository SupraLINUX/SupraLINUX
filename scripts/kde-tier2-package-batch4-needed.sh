...
campaign_fingerprint() {
  git -C "${ROOT}" show "$1:manifests/kde-tier2-package-campaign-batch4.json" |
...
for path in "${changed[@]}"; do
  case "${path}" in
    docs/*|scripts/validate_*.py|scripts/test-*.sh|manifests/kde-tier2-package-batch4-attempts.json)
      continue ;;
    manifests/kde-tier2-package-campaign-batch4.json)
      if ! git -C "${ROOT}" cat-file -e "${BEFORE}:${path}" 2>/dev/null; then
        echo "Tier 2 Batch 4 campaign is new."; exit 0
      fi
      if [[ "$(campaign_fingerprint "${BEFORE}")" != "$(campaign_fingerprint "${AFTER}")" ]]; then
        echo "Tier 2 Batch 4 semantic campaign input changed."; exit 0
      fi
      ;;
    scripts/run-kde-tier2-package-batch4.sh|scripts/plan-kde-tier2-package-batch4.py|scripts/validate-kde-tier2-batch4-retained-inputs.py|scripts/kde-tier2-package-batch4-needed.sh|.github/workflows/kde-tier2-package-batch4.yml)
      if has_runnable "${AFTER}"; then
        echo "${path}: Batch 4 implementation changed with runnable nodes."; exit 0
      fi
      ;;
    manifests/kde-tier2-package-contracts.json|manifests/kde-frameworks-tier2.json|manifests/kde-frameworks-tier2-dependencies.json|manifests/kde-frameworks-tier1.json|manifests/kde-dag.json)
      if has_runnable "${AFTER}"; then
        echo "${path}: Batch 4 consumed canonical input changed."; exit 0
      fi
      ;;
  esac
done
echo "No runnable Tier 2 Batch 4 semantic input changed."
exit 1
